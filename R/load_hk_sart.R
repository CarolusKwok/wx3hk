#' @title
#' HK weather - Downloading solar radiation information
#'
#' @description
...
#' @export
#'
#' @examples load_hk_sart()
load_hk_sart = function(time = seq.POSIXt(from = wx3hk::hk_time(),
                                          by = "-10 min",
                                          length.out = 1009),
                        lan = "en",
                        list_fail = TRUE,
  ...
  URL = tibble::tibble(time = time) %>%
    ...
    dplyr::mutate(LDate = paste0(sprintf("%04d", lubridate::year(Ltime)),
                  ...
                  URL = paste0("https://api.data.gov.hk/v1/historical-archive/get-file?url=https%3A%2F%2Fdata.weather.gov.hk%2FweatherAPI%2Fhko_data%2Fregional-weather%2Flatest_1min_solar",
                  ...
                  DIR = paste0(dir,
                               ...
                               "/", substr(PDate, 1, 6),
                               "/", PDate,
                               "/", "HK_SART", lan, "_", PDate, "_", PHour, ".csv")) %>%
