#' @title
#' HK weather - Downloading wind and gust information
#'
#' @description
...
#' @export
#'
#' @examples load_hk_wind()
load_hk_wind = function(time = seq.POSIXt(from = wx3hk::hk_time(),
                                          by = "-10 min",
                                          length.out = 1008),
                        lan = "en",
                        list_fail = TRUE,
  ...
  URL = tibble::tibble(time = time) %>%
    ...
    dplyr::mutate(LDate = paste0(sprintf("%04d", lubridate::year(Ltime)),
                  ...
                  URL = paste0("https://api.data.gov.hk/v1/historical-archive/get-file?url=https%3A%2F%2Fdata.weather.gov.hk%2FweatherAPI%2Fhko_data%2Fregional-weather%2Flatest_10min_wind",
                  ...
                  DIR = paste0(dir,
                               ...
                               "/", substr(PDate, 1, 6),
                               "/", PDate,
                               "/", "HK_WIND", lan, "_", PDate, "_", PHour, ".csv")) %>%
