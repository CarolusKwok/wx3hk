#' @title
#' HK weather - Downloading mean sea level pressure data
#'
#' @description
#' Downloading mean sea level pressure data from https://data.gov.hk/en/help/api-spec
#'
...
#' @export
#'
#' @examples load_hk_mslp()
load_hk_mslp = function(time = seq.POSIXt(from = Sys.time(), by = "-10 min", length.out = 1009),
                        lan = "en",
  ...
  wx3::load_file(data = URL,
                 ...
                 threshold = 100,
                 list_fail = list_fail,
                 title = "Mean Sea Level Pressure (HKO)")
}
