#' @title
#' HK weather - Downloading grass temperature data
#'
#' @description
#' Data downloaded from https://data.gov.hk/en/help/api-spec
#'
...
#' @export
#'
#' @examples load_hk_gtmp()
load_hk_gtmp = function(time = seq.POSIXt(from = Sys.time(), by = "-10 min", length.out = 1008),
                        lan = "en",
                        list_fail = T,
                        dir = getwd(),
                        attempt = 5L,
  ...
  URL = tibble::tibble(time = time) %>%
    ...
    dplyr::mutate(LDate = paste0(sprintf("%04d", lubridate::year(Ltime)),
                  ...
                  URL = paste0("https://api.data.gov.hk/v1/historical-archive/get-file?url=https%3A%2F%2Fdata.weather.gov.hk%2FweatherAPI%2Fhko_data%2Fregional-weather%2Flatest_1min_grass",
  ...
  wx3::load_file(
    ...
    threshold = 100,
    list_fail = list_fail,
    title = "Grass Temperature Information (HKO)"
  )
}
